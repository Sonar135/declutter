...
use PHPMailer\PHPMailer\Exception;
    ...
    if($query){
        ...
        if(    sendThankYouEmail($email, $name, $reason) ){
            echo json_encode([
                "status" => "success"
            ]);
        }
...
function sendThankYouEmail($toEmail, $item, $reason) { // Add username parameter
    try {
        ...
        // Content
        $mail->isHTML(true);
        $mail->Subject = 'Donation Rejection';
        ...
        <body>
            <div class="container">
                <h1>Apologies</h1>
                <p>Your donated item  <span> ' . htmlspecialchars($item) . '</span>  has been rejected</p>
